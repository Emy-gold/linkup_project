            ...
            <div class="grid-container">
                ...
                <div class="card">
                    ...
                    <a href="javascript:void(0)"
                       onclick="openVerifyModal(<%= universite.getId_universite() %>)"
                       class="btn">
                        Gérer
                    </a>
                </div>
...
<div id="verifyModal" class="modal-overlay">
    <div class="modal-content">
        <div class="modal-header">
            <h3 class="modal-title">Vérification de sécurité</h3>
            <p class="modal-description">Veuillez confirmer vos identifiants pour accéder à
                ce tableau de bord.</p>
        </div>
        <div class="form-group">
            <label for="verifyEmail" class="form-label">Email de l'université</label>
            <input type="email" id="verifyEmail" class="form-input"
                   placeholder="exemple@universite.com" required>
        </div>
        <div class="form-group">
            <label for="verifyPassword" class="form-label">Mot de passe</label>
            <input type="password" id="verifyPassword" class="form-input"
                   placeholder="Votre mot de passe" required>
        </div>
        <div id="verifyError" class="error-message"></div>
        <input type="hidden" id="verifyIdUniversite">
        <div class="modal-actions">
            <button type="button" class="btn-secondary"
                    onclick="closeVerifyModal()">Annuler</button>
            <button type="button" class="btn-primary"
                    onclick="submitVerification()">Valider</button>
        </div>
    </div>
</div>

<script>
    function openVerifyModal(idUniversite) {
        document.getElementById('verifyIdUniversite').value = idUniversite;
    ...
    function submitVerification() {
        const idUniversite = document.getElementById('verifyIdUniversite').value;
        ...
        xhr.open("POST", "<%= request.getContextPath() %>/verify-university-access", true);
            ...
            if (xhr.readyState === 4) {
                if (xhr.status === 200) {
                    try {
                        ...
                        if (response.success) {
                            window.location.href = "<%= request.getContextPath() %>/universite-dashboard?id_universite=" + idUniversite;
                        } else {
                            errorDiv.textContent = response.message || "Erreur de vérification.";
                            errorDiv.style.display = 'block';
                        }
                    } catch (e) {
                        console.error("Erreur parsing JSON", e);
                        errorDiv.textContent = "Erreur inattendue.";
                        errorDiv.style.display = 'block';
                    }
                } else {
        ...
        const params = "id_universite=" + encodeURIComponent(idUniversite) +
